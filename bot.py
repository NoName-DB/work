...
import config
from search import search_products, format_search_results, Product
...
price_tracking: dict[str, dict] = {}  # url -> tracking info
    ...
    try:
        if os.path.exists(config.TRACKING_FILE):
            with open(config.TRACKING_FILE, 'r', encoding='utf-8') as f:
                price_tracking.update(json.load(f))
    except Exception as e:
...
def add_price_tracking(user_id: int, product_id: str, product: dict) -> None:
...
def remove_price_tracking(user_id: int, url: str) -> bool:
...
async def cmd_track(message: Message) -> None:
    ...
    if not tracking:
        await message.answer("📊 У вас нет отслеживаемых товаров.\n\nИспользуйте команду /track <номер> для добавления товара из последнего поиска.", parse_mode="HTML")
        return
...
async def cmd_track_add(message: Message) -> None:
    """Обработчик команды /track <номер> - добавить товар для отслеживания"""
    user_id = message.from_user.id
    args = message.text.split()[1:]
    
    if not args:
        await message.answer("Используйте: /track <номер товара>\n\nСначала выполните поиск, затем укажите номер товара из результатов.", parse_mode="HTML")
        return
    
    try:
        item_index = int(args[0]) - 1  # 1-based to 0-based
    except ValueError:
        await message.answer("❌ Укажите корректный номер товара.", parse_mode="HTML")
        return
    
    # Здесь нужно получить последний результат поиска. Пока что упростим.
    await message.answer("Функция добавления товара для отслеживания будет реализована после поиска.\n\nИспользуйте кнопки под результатами поиска.", parse_mode="HTML")
...
async def handle_track_command(message: Message) -> None:
    ...
    if not command.startswith("track_"):
        return
    
    try:
        index = int(command.split("_")[1]) - 1  # track_1 -> index 0
    except (IndexError, ValueError):
        await message.answer("❌ Неверный формат команды. Используйте track_1, track_2 и т.д.", parse_mode="HTML")
        return
    
    if user_id not in last_search_results or not last_search_results[user_id]:
        await message.answer("Сначала выполните поиск командой /search или выберите быстрый поиск, затем используйте track_1, track_2 и т.д.", parse_mode="HTML")
        return
    
    products = last_search_results[user_id]
    if index < 0 or index >= len(products):
        await message.answer(f"❌ Неверный индекс товара. Доступны индексы от 1 до {len(products)}.", parse_mode="HTML")
        return
    ...
    while True:
        try:
            ...
            for key, tracking_info in list(price_tracking.items()):
                ...
                # Получаем текущую цену (упрощенная версия - в реальности нужно парсить сайт)
                # Для демонстрации просто симулируем небольшое изменение
                ...
                if abs(current_price - tracking_info["current_price"]) > 0.01:  # Минимальное изменение
                    ...
                    message = (
                        ...
                        f"Старая цена: {old_price} {tracking_info['currency']}\n"
                        f"Новая цена: {current_price} {tracking_info['currency']}\n"
